...
服务端收到请求后，会将自己的数字证书发送给客户端。数字证书包含 网站信息、公钥等内容，由权威的证书办法机构CA签发。

- 客户端生成一个随机数。
- 客户端使用服务器证书中的公钥加密随机数，得到密文。
- 客户端将密文发送给服务器端。
- 服务器端使用自己的私钥解密密文，得到随机数。
- 客户端和服务器端使用相同的随机数作为对称加密密钥。
...
写入一致性:
- 推送更新：当源服务器更新数据时，CDN会主动将更新内容推送到所有边缘节点。
- 失效策略：CDN会在每个节点上设置一个失效时间，当失效时间到来时，节点会重新从源服务器获取最新数据。
- 缓存更新协议：CDN可以使用缓存更新协议，例如HTTP的Cache-Control指令，来控制缓存的内容过期时间和更新策略。
  
读取一致性:
- 一致性哈希算法：CDN可以使用一致性哈希算法来将内容均匀地分布在所有边缘节点上，这样可以减少任何单个节点的负载，并提高读取的命中率。
- DNS解析：CDN可以使用DNS解析来将用户请求路由到最近的边缘节点，这样可以减少延迟并提高访问速度。
- 内容版本控制：CDN可以使用内容版本控制来管理不同版本的内容，这样可以确保用户始终获取最新版本的内容。
